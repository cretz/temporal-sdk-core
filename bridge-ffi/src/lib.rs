...
struct UserDataHandle(*const libc::c_void);
...
pub extern "C" fn tmprl_core_init(
    ...
    callback: unsafe extern "C" fn(*const libc::c_void, *mut tmprl_core_t, *const tmprl_bytes_t),
) {
...
pub extern "C" fn tmprl_core_shutdown(
    ...
    req_proto: *const u8,
    req_proto_len: libc::size_t,
    callback: unsafe extern "C" fn(*const tmprl_bytes_t),
) {
    ...
    core.tokio_runtime.clone().spawn(async move {
        core.shutdown().await;
        unsafe {
            callback(&*DEFAULT_SHUTDOWN_RESPONSE_BYTES);
        }
    });
}
...
pub extern "C" fn tmprl_register_worker(
    ...
    let req =
        match tmprl_core_t::decode_proto::<bridge::RegisterWorkerRequest>(req_proto, req_proto_len)
        {
            ...
            Err(message) => {
                ...
                unsafe {
                    callback(core.encode_proto(&resp).into_raw());
                }
                return;
            }
        };
    core.tokio_runtime.clone().spawn(async move {
        match core.register_worker(req).await {
            Ok(()) => unsafe {
                callback(&*DEFAULT_REGISTER_WORKER_RESPONSE_BYTES);
            },
            Err(err) => {
                let resp = bridge::RegisterWorkerResponse { error: Some(err) };
                unsafe { callback(core.encode_proto(&resp).into_raw()) };
            }
        }
    });
}
